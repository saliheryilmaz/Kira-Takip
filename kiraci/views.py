...
def kiraci_listesi(request):
    ...
    # ─── Grafik verisi: son 12 ay tahsilat (aylık) ───
    ay_adlari_kisa = ['', 'Oca', 'Şub', 'Mar', 'Nis', 'May', 'Haz',
                      'Tem', 'Ağu', 'Eyl', 'Eki', 'Kas', 'Ara']
    grafik_aylar = []
    grafik_odenen = []

    current_yil, current_ay = bugun.year, bugun.month
    for _ in range(12):
        ay_odenen = Odeme.objects.filter(
            kiraci__user=request.user,
            yil=current_yil,
            ay=current_ay
        ).aggregate(t=Sum('odenen_tutar'))['t'] or Decimal('0')

        grafik_aylar.insert(0, f"{ay_adlari_kisa[current_ay]} {str(current_yil)[2:]}")
        grafik_odenen.insert(0, float(ay_odenen))
    ...
    # ─── Grafik verisi: son 3 yıl tahsilat (yıllık) ───
    grafik_yillar = []
    grafik_yillik = []
    for y in range(bugun.year - 2, bugun.year + 1):
        yil_odenen = Odeme.objects.filter(
            kiraci__user=request.user,
            yil=y
        ).aggregate(t=Sum('odenen_tutar'))['t'] or Decimal('0')
        grafik_yillar.append(str(y))
        grafik_yillik.append(float(yil_odenen))

    # Kiracı bazlı alacak grafiği (aktif, borcu olanlar)
    kiraci_borc_listesi = []
    for k in aktif_qs:
        borc = k.toplam_borc()
        if borc > 0:
            kiraci_borc_listesi.append({'firma': k.firma_adi, 'borc': float(borc)})
    kiraci_borc_listesi.sort(key=lambda x: x['borc'], reverse=True)
    ...
    context = {
        ...
        'grafik_aylar_json':   json.dumps(grafik_aylar),
        'grafik_odenen_json':  json.dumps(grafik_odenen),
        'grafik_yillar_json':  json.dumps(grafik_yillar),
        'grafik_yillik_json':  json.dumps(grafik_yillik),
        'kiraci_borc_json': json.dumps(kiraci_borc_listesi),
        'kiraci_borc_listesi': kiraci_borc_listesi,
    }
...
def aylik_ozet(request):
    ...
    kiraciler = Kiraci.objects.filter(
        ...
        olusturulma_tarihi__date__lte=f'{yil}-{ay:02d}-28'
    )
    ozet_listesi = []
